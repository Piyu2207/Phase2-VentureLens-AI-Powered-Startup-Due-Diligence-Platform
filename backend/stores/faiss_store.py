...
class FAISSStore:
    ...
    def hybrid_search(self, query, startup_name, k=5):
        dense = self.dense_search(query, max(10, k * 5))
        sparse = self.bm25_search(query, max(10, k * 5))
        ...
        target = startup_name.strip().lower()

        for cid, item in items.items():

            score = (
            (1 / (60 + dr[cid]) if cid in dr else 0)
            + (1 / (60 + br[cid]) if cid in br else 0)
            )

            item_startup = item.get("startup", "").strip().lower()

            # Only keep evidence belonging to requested startup
            if item_startup != target:
                 continue
